...
const RecipePage = () => {
    ...
    const getRecipes = async () => {
        ...
        document.title = `Tasty - ${data.title}`;
    ...
    const _unitFormater = (amount: number, name: string) => {
        if (amount < 1) {
            ...
            // breaks code, not sure why, no errors occure as well. will check later
            // for (; test; ) {
            //     const check: number = initNumber / amount;
            //     if (check % 1 < 1) {
            //         initNumber++;
                ...
                {recipe?.diets && (
                    <div className="recipe_diets--list">
                        ...
                        {recipe.diets.map((diet, i) => (
                            <span className="recipe_diet" key={`${diet} diet`}>
                                <i>
                                    {diet}
                    ...
                    <div className="recipe_detail_ingredients">
                        ...
                        <ol className="recipe_ingredients">
                            {recipe?.extendedIngredients.map((ingredient) => (
                                <li className="recipe_ingredient" key={`${ingredient.nameClean} ingredient`}>
                    ...
                    <div className="recipe_detail_instructions">
                        ...
                        <ol className="recipe_instructions">
                            {recipe?.analyzedInstructions[0]?.steps.map((instruction, i) => (
                                <li className="recipe_instruction" key={`instruction step ${++i}`}>
